...
foreach ($placeprop in @('birtplac', 'chrplac', 'deatplac', 'buriplace')) {
    ...
    $allData | ?{$_.$placeprop -match $placeFilter} | %{new-object PSobject -property @{placeprop=$placeprop;name=$($_.given + " " + $_.surname);place=$_.$placeprop;date=sanitizeDate($_.$dateprop);fid=$_.fid}} | select placeprop, date, name, place, fid | sort date | Format-Table -autosize
}
...
$nearbymarriages | select placeprop, date, place, hubby_name, wifey_name, hubby_fid, wifey_fid | sort date | Format-Table -autosize
